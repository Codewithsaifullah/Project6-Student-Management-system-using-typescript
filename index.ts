...
class student {
  ...
  constructor(id: string, name: string, course: string[], fees: number) {
    this.id = id;
    this.name = name;
  ...
  if (action.ans === "Enroll in a Course") {
    ...
    if (studentNameCheck.includes(trimmedStudentName) === false) {
      if (trimmedStudentName !== "") {
        ...
        console.log(
          `Welcome To Sudent Mangment System, ${trimmedStudentName}!`
        );
        console.log(
          `Your are successfully login in our System with your StudentiId, ${studentId} \n That has automatically genrated by our System For you`
        );
      ...
      console.log(
        "Dear Student your has been already registered in our Database\n kindly Enter  your Complete for Enrollment!"
      );
    }
    ...
    if (studentList.length !== 0) {
      ...
      let foundStudent = studentList.find(
        (newStudent) => newStudent.name === selectedStudents.ans
      );
